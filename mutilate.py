...
def mutilate(chr,seq,muts, enspep, ensGene,codons, prefix, pep_lens):
    """
    This method accepts all the parsed input files and will identify the effect 
    of the mutations.  The outputs of running this method are:
    1. <prefix>_<chr>_mutilated.vcf - This is a vcf formatted file containing
       the same details as the input vcf, albeit with and additional semi-colon
       separated value in the INFO field describing the mutation in protein 
       space.
    2. <prefix>_<chr>_mutilated_<peplen>_mer.faa - This is a fasta formatted 
       file containing the peplen-mer peptides for submitting to MHC:ligand
       binding predictors.  A separate file will be created per peplen provided.
    3. <prefix>_<chr>_mutilated.log - This is the logfile for processing the
       input chromosome.
    """
    output=collections.Counter()
    output["out_file"] = open("_".join([prefix, chr, "mutilated.vcf"]), 'w')
    output["out_log"] = open("_".join([prefix, chr, "mutilated.log"]), 'w')
    for p in pep_lens:
        output[p] = open("_".join([prefix, chr, "mutilated", str(p), 
                         "_mer.faa"]), 'w')
     # This is where a lot of shit will go in
    [output[x].close() for x in output.keys()]
    return None
...
def main(args):
    ...
    chrom     = A list of chromosomes to be processed by the script. This allows
                for distribution of workload over multiple nodes on the cluster.
    codon     = A file containing the amino acid coding scheme of interest.  
                This has to be in a specific format. Refer parse_codon_scheme().
    ...
    prefix    = The desired prefix to append to output files.
    pept_lens = The lengths of peptide products that will be considered while 
    ...
    parser.add_argument('--in_ens', dest = 'in_ens', type = file_type, 
                       ...
                       default = "/inside/depot4/users/arjun/tools/mutilate/" +
    ...
    parser.add_argument('--prefix', dest = "prefix", type=str, 
                       ...
                       default = "Mutilated", required = False)
    ...
    all_chroms = ["".join(["chr",str(x)]) for x in range(1,23) + 
                         ["X", "Y", "M_rCRS"]]
    if params.chrom == "all":
        chrom_in = all_chroms
    else:
    ...
    print("PROGRESS: Reading in ensGene file to memory...", end = "", 
          file = sys.stderr)
    ensG_file = parse_ensGene_file(params.in_ens)
    params.in_ens.close()
    print("\tDONE", file = sys.stderr)
    ...
    for protein in prot_sequences:
        prot[fasta.id] = list(protein.seq.tostring())
    ...
    for fasta in fasta_sequences:
        ...
        mutilate(name,seq,mutations[name], prot, ensG_file,codon_scheme, 
                 params.prefix, params.pep_lens.split(","))
        print("\tDONE", file = sys.stderr)
    params.in_gen.close()
